...
const CACHE_NAME = 'grocery-delivery-v3';
const RUNTIME_CACHE = 'grocery-delivery-runtime-v3';
...
self.addEventListener('fetch', (event) => {
    ...
    // Ignorar requests a la API (siempre ir a red sin cachear)
    // Permitir peticiones a la API backend (puerto 8443)
    if (url.pathname.startsWith('/api/') || url.port === '8443') {
        return;
    }

    // Ignorar chrome extensions y otros requests no relacionados
    // Permitir requests al mismo host pero diferente puerto (backend en 8443)
